...
def fit_one_epoch(model_rpn, model_all, loss_history, callback, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, anchors, bbox_util, roi_helper):
    ...
    with tqdm(total=epoch_step,desc=f'Epoch {epoch + 1}/{Epoch}',postfix=dict,mininterval=0.3) as pbar:
        for iteration, batch in enumerate(gen):
            ...
            if loss_class[4] > 100 or np.isnan(loss_class[4]):
                print(X, np.array(roi_inputs))
                print(Y[0], Y[1])
                print(np.array(out_classes), np.array(out_regrs))
                raise ValueError("Got unreasonable loss value (too high or NaN). Exit!")
    ...
    # model_all.save_weights(log_dir + '/ep%03d-loss%.3f-val_loss%.3f.h5' % (epoch + 1, total_loss / epoch_step, val_loss / epoch_step_val))
