...
class FRCNN(object):
    ...
    _defaults = {
        "model_path"    : 'a-place-holder.h5',
        "classes_path"  : 'a-place-holder.txt',
        "backbone"      : "resnet50",
        "confidence"    : 0.5,
        "nms_iou"       : 0.3,
    ...
    def get_map_txt(self, image_id, image, class_names, map_out_path, model_id):
        f = open(os.path.join(map_out_path, f"detection-results-model{model_id}/"+image_id+".txt"),"w") 
        image_shape = np.array(np.shape(image)[0:2])
        input_shape = get_new_img_size(image_shape[0], image_shape[1])
        image       = cvtColor(image)
        image_data  = resize_image(image, [input_shape[1], input_shape[0]])
        ...
        # return the original image if no object detected
        if len(results[0])<=0:
            return 
